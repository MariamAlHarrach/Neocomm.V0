...
def Create_Connectivity_Matrix(C,inputpercent,NB_DPYR,NB_Th,Cellpos,*args):
    #C is column morphology

    #output dict
    Conx={}
    if not Create_Connectivity_Matrix.__code__.co_argcount < 5:
        # if not seed == 0:
        #     np.random.seed(seed)
        ...
        List_cellsubtypes = copy.deepcopy(C.List_celltypes)
                            ...
                            if ((v == cell) and (subl == l) and (C.List_celltypes[l][cell] != 1)):
                                ...
                                if isconnected == 1:
                                    ...
                                    if Neighbor.type == 0:  # excitatory Input
                                        AMPAcells[index] = v + np.sum(SLayer_nbCells[subl])
                                        GABAcells[index] = -1
                                        GABASoma[index] = '0'
                                    else:
                                        # inhibitory input
                                        GABAcells[index] = v + np.sum(SLayer_nbCells[subl])
                                        AMPAcells[index] = -1
        ...
        print('Error!!!!!!: Not enough arguments for the computing of connectivity')
