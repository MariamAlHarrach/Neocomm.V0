...
def get_Variable_Names():
    return ['g_leak', 'g_Na_s', 'g_KDR_s', 'g_CaL_s', 'g_AHP_s', 'g_AHP_BK_s', 'g_m_s', 'g_KNa',
...
class pyrCellCA1:
    ...
    def setParameters(self):
        ...
        self.y[3] = self.m_KDR_inf_CA1(
            self.y[0])  # alpha_n_CA1(self.y[0])/(alpha_n_CA1(self.y[0])+beta_n_CA1(self.y[0]));
    ...
    def updateParameters(self):
        ...
        self.I_CaT_d = self.I_CaT_CA1(self.g_CaT_d, self.y[10], self.y[11], self.fonc_Ca_s, self.fonc_ghk_s, self.y[1],
                                      self.E_Ca)
        ...
        self.I_leak_s = self.I_leak_CA1(self.g_leak, self.PERCENT, self.y[0], self.E_leak)
        self.I_leak_d = self.I_leak_CA1(self.g_leak, self.PERCENT, self.y[1], self.E_leak)
    ...
    def derivT(self):
        ...
        self.dVs_dt = 1. / self.Cm * (
                    -self.I_KNa - self.I_Na_s - self.I_KDR_s - self.I_CaL_s - self.I_AHP_s - self.I_AHP_BK_s - self.I_m_s - self.I_leak_s - self.I_synSoma - (
                        self.g_c_SD / self.p * (self.Vs_ - self.Vd_)) + self.I_soma_stim)
        self.dVd_dt = 1. / self.Cm * (
                    -self.I_Na_d - self.I_KDR_d - self.I_KA_d - self.I_CaT_d - self.I_CaR_d - self.I_h_d - self.I_AHP_d - self.I_AHP_BK_d - self.I_m_d - self.I_leak_d - self.I_synDend - (
                        self.g_c_SD / (1 - self.p) * (-self.Vs_ + self.Vd_)) + self.I_dend_stim)
        ...
        self.dNa_intra_dt = -self.alpha_Na * (self.A_s * (self.I_Na_s)) - self.Rpump * (
...
#    def I_AMPA2(self, Vpre, PreSynaptic_Soma_Dend_AMPA):
...
#        print(len(self.s_AMPA))
    ...
    def computeI_AMPA(self, Vpre):
        ...
        # K4
        self.s_AMPA = self.s_AMPAo + self.dt / 6. * (
                    self.ds_AMPA1 + 2 * self.ds_AMPA2 + 2 * self.ds_AMPA3 + self.derivs_AMPA_CA1())
        # y at t+dt
        return self.s_AMPA
    ...
    def computeI_GABA(self, Vpre):
        ...
        # K4
        self.s_GABA = self.s_GABAo + self.dt / 6. * (
                    self.ds_GABA1 + 2 * self.ds_GABA2 + 2 * self.ds_GABA3 + self.derivs_GABA_CA1())
        # y at t+dt
        return self.s_GABA
