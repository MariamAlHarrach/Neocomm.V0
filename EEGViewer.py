...
class lfpViewer_EEG(QMainWindow):
    ...
    def updateslider(self):
        self.horizontalSliders.setMinimum(0)
        self.horizontalSliders.setMaximum(np.ceil((self.t[-1]-self.t[0]) / int(self.e_win.text())) - 1)
    ...
    def SaveRes_fun(self):
        fileName = QFileDialog.getSaveFileName(caption='Save parameters', filter=".bin (*.bin);;.edf (*.edf);;.csv (*.csv);;.data (*.data);;.mat (*.mat);;.dat (*.dat)")
        if (fileName[0] == ''):
            return
...
class EEG_plot(QGraphicsView):
    ...
    def update(self):
        win_num = self.parent.horizontalSliders.value()
        self.figure.clear()
        # self.figure.canvas.callbacks.connect('pick_event', self.pick_handler)
