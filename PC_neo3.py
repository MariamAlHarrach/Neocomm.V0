...
def get_Variable_Names():
    return  ['g_leak_s','g_leak_d','g_leak_a','g_Na_s','g_KDR_s','g_CaL_s','g_AHP_s','g_AHP_BK_s','g_m_s','g_KNa',
             ...
             'Cm','Cm_d','p_SD' , 'p_SA','g_c_SA','g_c_SD','PERCENT','Noise']
...
# I_h: hyperpolarisation activated current
    ...
    def __init__(self,Layer):
        ...
        #Electric field
        self.Lambda_d = np.zeros(3,)
        #self.Lambda_d[2] = -1
        self.Lambda_s = np.zeros(3,)
        #self.Lambda_s[2] = -1
        self.Lambda_a = np.zeros(3,)
        #self.Lambda_a[2] = 1
        self.E_d = np.zeros(3,)
        self.E_s = np.zeros(3,)
        self.E_a = np.zeros(3,)
    ...
    def projection_E(self,E_d, E_s, E_a):
        self.E_d = E_d
        self.E_s = E_s
        self.E_a = E_a

        # self.proj_mod_d = np.linalg.norm(self.Lambda_d * np.dot(self.E_d, self.Lambda_d) / np.dot(self.Lambda_d, self.Lambda_d)) * np.sign(np.linalg.norm((np.dot(self.E_d, self.Lambda_d) / np.dot(self.Lambda_d, self.Lambda_d)) - self.E_d))
        # self.proj_mod_s = np.linalg.norm(self.Lambda_s * np.dot(self.E_s, self.Lambda_s) / np.dot(self.Lambda_s, self.Lambda_s)) * np.sign(np.linalg.norm((np.dot(self.E_s, self.Lambda_s) / np.dot(self.Lambda_s, self.Lambda_s)) - self.E_s))
        # self.proj_mod_a = np.linalg.norm(self.Lambda_a * np.dot(self.E_a, self.Lambda_a) / np.dot(self.Lambda_a, self.Lambda_a)) * np.sign(np.linalg.norm(self.Lambda_a * np.dot(self.E_a, self.Lambda_a) / np.dot(self.Lambda_a, self.Lambda_a)) - self.Lambda_a))


        projection = self.Lambda_d * np.dot(self.E_d, self.Lambda_d) / np.dot(self.Lambda_d, self.Lambda_d)
        norm_projection = np.linalg.norm(projection)
        projection_moi_vect = projection + self.Lambda_d
        norm_vect_projectOnto = np.linalg.norm(projection_moi_vect)
        signe = np.sign(norm_vect_projectOnto - norm_projection)
        self.proj_mod_d = norm_projection * signe
        projection = self.Lambda_s * np.dot(self.E_s, self.Lambda_s) / np.dot(self.Lambda_s, self.Lambda_s)
        norm_projection = np.linalg.norm(projection)
        projection_moi_vect = projection + self.Lambda_s
        norm_vect_projectOnto = np.linalg.norm(projection_moi_vect)
        signe = np.sign(norm_vect_projectOnto - norm_projection)
        self.proj_mod_s = norm_projection * signe
        projection = self.Lambda_a * np.dot(self.E_a, self.Lambda_a) / np.dot(self.Lambda_a, self.Lambda_a)
        norm_projection = np.linalg.norm(projection)
        projection_moi_vect = projection + self.Lambda_a
        norm_vect_projectOnto = np.linalg.norm(projection_moi_vect)
        signe = np.sign(norm_vect_projectOnto - norm_projection)
        self.proj_mod_a = norm_projection * signe


        self.Vtm_d = self.proj_mod_d
        self.Vtm_s = self.proj_mod_s
        self.Vtm_a = self.proj_mod_a

    def projection_synapse_E(self,E_d, E_s, E_a):
        self.E_d = E_d
        self.E_s = E_s
        self.E_a = E_a
        self.Vtm_d = self.proj_mod_d
    ...
    def update_EField_Stim(self,f):

        self.Vtm_d =  (self.proj_mod_d * f) * self.OnOff
        self.Vtm_s =  (self.proj_mod_s * f) * self.OnOff
        self.Vtm_a =  (self.proj_mod_a * f) * self.OnOff
    ...
    def derivT(self):
        ...
        self.dVs_dt = 1. / self.Cm * (-self.I_KNa - self.I_Na_s - self.I_KDR_s - self.I_CaL_s - self.I_AHP_s - self.I_AHP_BK_s - self.I_m_s - self.I_leak_s - self.I_synSoma - (self.g_c / self.p_SD * ((self.Vs_+self.Vtm_s)- (self.Vd_+self.Vtm_d))) - (self.g_c / self.p_SA * ((self.Vs_+self.Vtm_s)- (self.Va_+self.Vtm_a))) + self.I_soma_stim) + self.Noise
        self.dVd_dt = 1. / self.Cm_d * (-self.I_Na_d - self.I_KDR_d - self.I_KA_d - self.I_CaT_d - self.I_CaR_d - self.I_h_d - self.I_AHP_d - self.I_AHP_BK_d - self.I_m_d - self.I_leak_d - self.I_synDend - (self.g_c / (1 - self.p_SD) * (-(self.Vs_+self.Vtm_s) + (self.Vd_+self.Vtm_d))) + self.I_dend_stim)
        self.dVa_dt = 1. / self.Cm * (-self.I_Na_a - self.I_KDR_a -self.I_leak_a  - self.I_synAis - (self.g_c /(1-self.p_SA) * ((self.Va_+self.Vtm_a)- (self.Vs_+self.Vtm_s))) + self.I_axon_stim)
    ...
    def I_Na_CA1(self, g_Na, m_inf, h, Vs, E_Na):
        ...
        #
        #    sigma = PERCENT * g_Na;
        #    g_n = bruitGaussien(sigma,g_Na);
        #
        # return g_n*m_inf*m_inf*h*(Vs-E_Na);
    ...
    def computeI_GABA(self, Vpre):
        self.F_fonc = 10. * self.F_fonc_CA1(Vpre)
        # rk4
    ...
    def computeI_NMDA(self,Vpre,t,Vs_d):
        ...
        if (q > self.deadTime):
           ...
           pass
            ...
            self.C = 0.
        if (self.C > 0):
            self.Rinf = self.Cmax * self.alpha / (self.Cmax * self.alpha + self.beta)
            self.Rtau = (1. / (self.Cmax * self.alpha + self.beta))
            self.Ra = self.Rinf + (self.R0-self.Rinf) * np.exp(-(t-self.lastRelease) / self.Rtau)
        else:
            self.Ra = self.R1 * np.exp(-1.0 * self.beta * (t-(self.lastRelease+self.Cdur)))

        B = 1. / (1 + np.exp(0.062 * (-Vs_d)) * self.mg / 10.)  # 3.57); # mgblock(Vm)

        return self.Ra * B * self.g_NMDA * (Vs_d - self.E_NMDA)
