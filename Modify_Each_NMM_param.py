...
class Modify_1_NMM(QMainWindow):
    ...
    def set_param(self):
        ...
        self.edit_model =[]
        for idx_p, p in enumerate(self.Dict_Param):
            edit =[]
            list_variable = []
            for key, value in p.items():
                edit.append(str(value))
                list_variable.append(key)
            layout_NMM_var, Edit_List_NMM_var = Layout_groupbox_Label_Edit(labelGroup ='', label = list_variable,edit =edit, name=self.List_Names[idx_p], color=self.List_Color[idx_p], width = 150)
